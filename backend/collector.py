...
def get_train_status(stations):
    ...
    for station in stations:
        ...
        sch = station.get("schArr")
        actual = station.get("arr")

        if not sch or not actual:
            return "On Time"

        try:
            scheduled = datetime.fromisoformat(sch)
            predicted = datetime.fromisoformat(actual)
...
for train_group in api.values():
    ...
    for train in train_group:
        ...
        # Keep only trains inside Mohawk Subdivision
        if not (
        ...
        status = get_train_status(stations)
        ...
        for station in stations:
            ...
            status = str(station.get("status", "")).lower()
